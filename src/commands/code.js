    ...
    if (message.content === "!randomcode") {
      ...
      if (
        !message.member.roles.cache.some(
          (role) => role.id === "1092611286010966046" // ID FUNDADOR ROLE
        ) &&
        message.author.id !== "405096240888676352" // ID ROCKET
      ) {
      ...
      if (
        !message.member.roles.cache.some(
          (role) =>
            role.id === "1092611286010966046" || // ID FUNDADOR ROLE
            role.id === "1092622024540700802" // ID RUDE ROLE
        ) &&
        message.author.id !== "405096240888676352" // ID ROCKET
      ) {
        message.reply("Não tens permissão para usar o comando.");
      ...
      if (
        !message.member.roles.cache.some(
          (role) => role.id === "1092611286010966046" // ID FUNDADOR
        ) &&
        message.author.id !== "405096240888676352" // ID ROCKET
      ) {
        message.reply("Não tens permissão para usar o comando.");
